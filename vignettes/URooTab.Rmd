---
title: "Conduct Unit Root Using EViews Routines"
author: "Sagiru Mati, PhD"
date: '`r format(Sys.Date(), "%A: %B %d,  %Y")`'
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{URooTab: A Seamless Integration of EViews and R}
...
as shown below :
    ...
    #| fig.cap: "EViews graphs imported automatically by fig-URooTab chunk"

        'This program is created in R Markdown with the help of URooTab package
      
      wfcreate(page=URooTabPage,wf=URooTab_workfile) m 2000 2022
      for %y URooTab package page1 page2
      pagecreate(page={%y}) URooTab m 2000 2022
      next
      pageselect URooTabPage
      rndseed 123456
      genr y=@cumsum(nrnd)
      genr x=@cumsum(nrnd)
      equation ols.ls y c x
      freeze(OLSTable,mode=overwrite) ols
      freeze(URooTab_Plot,mode=overwrite) y.line
      wfsave URooTab_workfile
    ```  

<img src="../inst/figures//URooTabURooTabpage-xx.png" alt="EViews graphs imported automatically by fig-URooTab chunk" width="45%" height="20%" /><img src="../inst/figures//URooTabURooTabpage-yy.png" alt="EViews graphs imported automatically by fig-URooTab chunk" width="45%" height="20%" />
<p class="caption">
Figure 7.1: EViews graphs imported automatically by fig-URooTab chunk
</p>
...
The program will also save an EViews workfile named `URooTab_workfile`
...
`` `r knitr::inline_expr('URooTab$URooTabpage_ols$r2')` ``. We can obtain the table object by
`chunkLabel$pageName_tableName`. Therefore,
`URooTab$URooTabpage_olstable` will give us the `OLSTable` object as
dataframe. Note the underscore (`_`) between the `pageName` and
`equationName`, and between the `pageName` and `tableName`.

    URooTab$URooTabpage_ols$r2
    #> [1] 0.044951
    URooTab$URooTabpage_ols$aic
    #> [1] 4.310163
    K = URooTab$URooTabpage_olstable[c(6, 8, 9), 1:5]
    colnames(K) = NULL
...
the existing EViews workfile.
    ...
    create_object(wf = "URooTab_workfile", action = "equation", action_opt = "",
        object_name = "eviews_equation", view_or_proc = "ls", options_list = "",
        arg_list = "y ar(1)")

    create_object(wf = "URooTab_workfile", object_name = "x1", object_type = "series",
        expression = "y^2")
...
To create graph from existing EViews series objects:
    ...
    eviews_graph(wf = "URooTab_workfile", page = "URooTabPage", series = "x y",
        mode = "overwrite", graph_procs = "setelem(1) lcolor(red) lwidth(4)",
        graph_options = "m")

<img src="../inst/figures//eviewsgraph-URooTabpage-x.png" alt="Graphs of existing EViews series objects imported by fig-eviewsGraph chunk" width="45%" height="30%" /><img src="../inst/figures//eviewsgraph-URooTabpage-y.png" alt="Graphs of existing EViews series objects imported by fig-eviewsGraph chunk" width="45%" height="30%" />
<p class="caption">
...
<img src="../inst/figures//eviewsgraph2-URooTabpage-xy.png" alt="Scatter graph along with histogram" width="90%" height="80%" />
<p class="caption">
...
formats by `eviews_pagesave()` function.
    ...
    eviews_pagesave(wf = "URooTab_workfile", page = "URooTabPage",
...
R.
    ...
    eviews_wfcreate(wf = "eviews_wfcreate", page = "URooTabPage",
...
Create a workfile from a dataframe
    ...
    eviews_wfcreate(source_description = Data, wf = "eviews_wfcreate1",
        page = "URooTab_page", frequency = "m", start_date = "1990")
...
`eviews_wfsave()` in function in R.
    ...
    eviews_wfsave(wf = "URooTab_workfile", source_description = "wfsave.csv")
...
Import EViews equation data members into R, R Markdown or Quarto.
    ...
    import_equation(wf = "URooTab_workfile", page = "URooTabPage",
        equation = "OLS")
...
Import EViews graph objects(s) into R, R Markdown or Quarto.
    ...
    import_graph(wf = "URooTab_workfile")

<img src="../inst/figures//fig-importgraph-URooTabpage-xx.png" alt="EViews graphs imported using import\_graph() function" width="45%" height="20%" /><img src="../inst/figures//fig-importgraph-URooTabpage-yy.png" alt="EViews graphs imported using import\_graph() function" width="45%" height="20%" />
<p class="caption">
...
function. Therefore, we can include the
    ...
    import_kable(wf = "URooTab_workfile", page = "URooTabPage", table = "OLSTable",
...
objects can be accessed via `eviews$pageName`.
    ...
    import_series(wf = "URooTab_workfile")
...
To import the series as an `xts` object:
    ...
    import_series(wf = "URooTab_workfile", series = c("x", "y"),
        class = "xts")
...
To import all table objects across all pages
    ...
    import_table(wf = "URooTab_workfile")

To import specific table objects, for example `OLSTable`

    import_table(wf = "URooTab_workfile", table = "OLStable")

To import table objects on specific pages

    import_table(wf = "URooTab_workfile", page = " URooTabPage")
...
To import all equation, graph, series and table objects across all pages
    ...
    import_workfile(wf = "URooTab_workfile")

<img src="../inst/figures//fig-importworkfile-URooTabpage-xx.png" alt="EViews graphs automatically imported by import\_workfile() function" width="45%" height="20%" /><img src="../inst/figures//fig-importworkfile-URooTabpage-yy.png" alt="EViews graphs automatically imported by import\_workfile() function" width="45%" height="20%" />
<p class="caption">
...
thanks to `rwalk()` function.
    ...
    rwalk(wf = "URooTab_workfile", series = "X Y Z", page = "", rndseed = 12345,
...
`file->New File->R Markdown-> From Template->URooTab`.
...
[Github](https://github.com/sagirumati/URooTab/tree/master/inst/examples/).
