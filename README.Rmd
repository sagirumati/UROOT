...
The `uroot()` function reports all the available test (ADF and PP) at once. It is more suitable for Quarto document, which has both `tbl-cap` and `tbl-subcap` chunk options. To produce Table \@ref(tab:URooTab)

Notice the chunk option `results: asis` because `uroot()` is designed to print all the tables (ADF and PP) in the chunk.
...
adf(dataFrame,format = "latex",caption = "ADF Unit Root Tests for x, y and Z") %>% cat
```
